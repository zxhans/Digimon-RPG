...
namespace Digimon_Project.Network.Handlers.Map
{
    ...
    [PacketHandler(Type = PacketType.PACKET_DIGIMON_INCR_SKILL, Connection = ConnectionType.Map)]
    public class HANDLE_DIGIMON_INCR_SKILL : Handler<Client>
    {
        public override void Handle(Client sender, InPacket packet)
        {
            ...
            foreach(Digimon d in sender.Tamer.Digimon)
            {
                if(d != null && d.Id == ID)
                {
                    d.AddPointSkill(estage, skill);
                    sender.Connection.Send(new Packets.PACKET_DIGIMON_INCR_SKILL(d, estage, skill));
                    return;
                }
            }
        }
